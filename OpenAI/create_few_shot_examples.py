...
if __name__ == "__main__":
    ...
    # get the mapping:
    mapping = get_mapping('mapping.txt')
    # have to reverse the mapping, bc we wanna decode
    mapping = {v: k for k, v in mapping.items()}

    # read in the label and text files, create two lists
    filepath_labels = os.path.join(data_dir, lang, 'train_labels.txt')
    filepath_texts = os.path.join(data_dir, lang, 'train_text.txt')
    val_labels = []
    val_texts= []
    ...
    example_filepath = os.path.join(data_dir, lang, 'examples.jsonl')
